        ...
        public function load(filename:String, resourceType:Class, onLoaded:Function = null, onFailed:Function = null, forceReload:Boolean = false):Resource
        {
            ...
            if (!resource)
            {
                ...
                for (var rp:int = 0; rp < resourceProviders.length; rp++)
                {
                    ...
                    {
                        resource = (resourceProviders[rp]
                                as
                                IResourceProvider
                                ).
                                getResource(filename, resourceType, forceReload);
                    }
                }
            ...
            else
            {
                ...
                if (onLoaded != null)
                {
                    resource.addEventListener(ResourceEvent.LOADED_EVENT, function (event
                :
                    Event
                ):
                    void
                ...
                if (onFailed != null)
                    resource.addEventListener(ResourceEvent.FAILED_EVENT, function (event:Event):void
                    {
                        onFailed(resource);
                    });
            }
